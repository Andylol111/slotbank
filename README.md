...
Point Oh My Pi at `http://127.0.0.1:8080`. Run `slotbank omp --model ~/Models/Qwen3.8-27B-4bit --thinking --vision` (no weight load) or `slotbank serve` — both write static `llama.cpp` / `lm-studio` rows into `~/.omp/agent/models.yml`. Then **`omp models refresh`**. Wait until serve prints **`ready`** before sending — a `hi` during load used to 400 as `still loading`. Start omp from `~` or `/tmp`, not this checkout, and prefer **`omp --tools read`**: the default ~31-tool catalog plus a project (`footer ↳ name`) is why a first `hi` sat at 80–112% of 33K. Serve **400s** a prompt over 8192 tokens (`SLOTBANK_MAX_PROMPT`; `0` disables) — a 45%/33K OMP footer is ~15k and jetsams mid-prefill as **Connection error**. **503s** while weights load. Picker `llama.cpp/Qwen3.8-27B-4bit` has tools off (fast hi); `llama.cpp/Qwen3.8-27B-4bit-agent` is the tool/thinking door. Snippet: [`examples/omp-qwen38.yml`](examples/omp-qwen38.yml). A sibling `Qwen3.8-27B-MTP-4bit` is auto-attached if you omit `--draft`. For 30+ tok/s / less RAM, use the 4B snippet below.
