...
Point Oh My Pi at `http://127.0.0.1:8080`. Run `slotbank omp --model ~/Models/Qwen3.8-27B-4bit --thinking --vision` (no weight load) or `slotbank serve` — both write static `llama.cpp` / `lm-studio` rows into `~/.omp/agent/models.yml`. Then **`omp models refresh`**. Wait until serve prints **`ready`** before sending. Start omp from an **empty directory** — `mkdir /tmp/sb-hi && cd /tmp/sb-hi && omp --no-tools`. Do not start from this checkout, and do not start from `/tmp` if `/tmp/<repo>` exists: OMP’s footer `🗑 /tmp ↳ llama.cpp-dflash2` means it injected that child git repo (measured **39040** tokens on `hi`). Serve **400s** a prompt over 8192 tokens (`SLOTBANK_MAX_PROMPT`; `0` disables). **503s** while weights load. Picker `llama.cpp/Qwen3.8-27B-4bit` has tools off (fast hi); `llama.cpp/Qwen3.8-27B-4bit-agent` is the tool/thinking door. Snippet: [`examples/omp-qwen38.yml`](examples/omp-qwen38.yml). A sibling `Qwen3.8-27B-MTP-4bit` is auto-attached if you omit `--draft`. For 30+ tok/s / less RAM, use the 4B snippet below.
